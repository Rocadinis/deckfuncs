...
def search(card, deckVar, handVar):
    """
    Void - Adds a card from a deck (deckVar) to a hand (handVar). 'card' can be treated as, for example, a variable or list element. Only one card of the given name is taken, not all occurences!
...
def bury(card, deckVar, gyVar):
    """
    Void - Sends a card from a deck (deckVar) to a graveyard / discard pile (gyVar). 'card' can be treated as, for example, a variable or list element. Only one card of the given name is taken, not all occurences!
